...
Distribution K8s provisionnée & k3s v1.31.4+k3s1 \\
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Tests du Cluster k3s}
% ─────────────────────────────────────────────────────────────────────────────

\begin{table}[H]
\centering
\caption{Résultats des tests du cluster k3s}
\label{tab:k8s_tests}
\begin{tabularx}{\linewidth}{lXll}
\toprule
\textbf{TC} & \textbf{Cas de test} & \textbf{Attendu} & \textbf{Résultat} \\
...
TC-K02 & Flux de progression SSE & Toutes les étapes atteignent l'état « complet » & \cmark \\
...
TC-K05 & Le worker accède à Internet via le NAT maître & Installation k3s agent réussie sur le worker & \cmark \\
TC-K06 & \texttt{kubectl get nodes} affiche tous les nœuds Ready & STATUS : Ready pour maître et workers & \cmark \\
...
TC-K11 & Terminal SSH navigateur vers le nœud maître & Shell interactif ouvert dans xterm.js & \cmark \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{lstlisting}[language=bash,
  caption={Sortie de kubectl get nodes sur le cluster k3s provisionné},
  label={lst:kubectl_nodes}]
$ kubectl get nodes -o wide
NAME                     STATUS   ROLES                  AGE   VERSION
k3s-cluster-master       Ready    control-plane,master   8m    v1.31.4+k3s1
k3s-cluster-worker-1     Ready    <none>                 5m    v1.31.4+k3s1
\end{lstlisting}
...
\screenshotplaceholder{kubectl get nodes --- les deux nœuds k3s en statut Ready avec la version v1.31.4+k3s1}

% ─────────────────────────────────────────────────────────────────────────────
\section{Tests d'Autoguérison et de Synchronisation}
% ─────────────────────────────────────────────────────────────────────────────

\begin{table}[H]
\centering
\caption{Résultats des tests d'autoguérison et de synchronisation}
\label{tab:selfheal_sync_tests}
\begin{tabularx}{\linewidth}{lXll}
\toprule
\textbf{TC} & \textbf{Cas de test} & \textbf{Attendu} & \textbf{Résultat} \\
\midrule
TC-SH01 & Activer l'autoguérison dans l'assistant + paiement & Option incluse dans le total (\$15/mois) & \cmark \\
TC-SH02 & Enregistrement spoke auprès du hub après provisionnement & Statut \texttt{registered} en BD & \cmark \\
TC-SH03 & Cluster provisionné sans autoguérison & Champ \texttt{enable\_self\_healing} = false & \cmark \\
TC-SY01 & Sync : VM supprimée depuis la console HCS & Enregistrement VM supprimé de la BD & \cmark \\
TC-SY02 & Sync : cluster supprimé hors plateforme & Enregistrement cluster supprimé de la BD & \cmark \\
TC-SY03 & Import : répertoire Terraform sans fiche BD & Enregistrement stub créé avec IP et config & \cmark \\
TC-SY04 & Import : répertoire Terraform avec état vide & Aucun enregistrement créé & \cmark \\
\bottomrule
\end{tabularx}
\end{table}
...
Provisionnement cluster k3s avec réseau pour workers privés
  & TC-K01 à TC-K11 validés ; cluster 2 nœuds Ready sur HCS & \cmark Atteint \\
Autoguérison hub-spoke et synchronisation cloud
  & TC-SH01 à TC-SH03 et TC-SY01 à TC-SY04 validés & \cmark Atteint \\
